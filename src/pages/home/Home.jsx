    ...
    <div className="container home-con">
              ...
              <div className="col-md-10">
                <div className="card">
                  ...
                  <div className="card-header">
                    <Link to="/adddevs"><button className='btn btn-warning'>Add Devs</button></Link>
...

                          {
                            formData.map((item, index)=>{
                              ...
                              return(
                                ...
                                <td>{index+1}</td>
                                <td><img src={item.photo} className='user-photo' /></td>
                                <td>{item.name}</td>
                                <td> {item.location} </td>
                                <td> {item.skill} </td>
                                <td>
                                  <button className='btn btn-sm btn btn-info m-1' onClick={()=>handleEditShow(item.id)}> <CiEdit/> </button> 
                                  <button className='btn btn-sm btn btn-danger m-1' onClick={() => handleDelete(item.id) }> <MdDeleteSweep/> </button>
                                  <button className='btn btn-sm btn btn-primary m-1' onClick={()=> handleSingleShow(item.id)} > <AiOutlineEyeInvisible/> </button>
                                </td>
                              </tr>
                              )
          ...
          <Row>
              <Col md={5}>
                <Modal show={editModalShow} onHide={seteditModalShow} centered>
                  <Modal.Header>
                    <h3>Edit Devs</h3>
                    <Button className='btn btn-close'  variant='danger' onClick={()=> seteditModalShow()} ></Button>
                  </Modal.Header>
                  <Modal.Body>
                  <form onSubmit={handleDevsFormEdit}> 
                        <div className="item my-3">
                            <label htmlFor=""> Name </label>
                            <input type="text" name="name" className="form-control" onChange={handleEditInputChange}  value={editInput.name} />
                        </div>

                        <div className="item my-3">
                            <label htmlFor="">Location</label>
                            <input type="text" name="location" className="form-control"  onChange={handleEditInputChange} value={editInput.location}  />
                        </div>

                        <div className="item my-3">
                            <label htmlFor="">Skill</label>
                            <input type="text" name="skill" className="form-control" onChange={handleEditInputChange} value={editInput.skill}  />
                        </div>

                        <div className="item my-3">
                            <label htmlFor="">Choose Your Photo</label>
                            <input type="link" name="photo" className="form-control" onChange={handleEditInputChange} value={editInput.photo}  />
                        </div>
      ...
      <div className="single">
        <Container>
          <Row>
              <Col>
                <Modal show={singleModalShow} onHide={setSingleModelShow} centered>
                  <Modal.Header>
                    <h3>Single Devs</h3>
                    <Button className='btn btn-close'  variant='danger' onClick={setSingleModalHide} ></Button>
                  </Modal.Header>
                  <Modal.Body>
                    <Card>
                        <CardBody>
                          <div className="singleData">
                            <div className="img">
                              <img src={singData.photo} className='w-100' style={{height: "400px", objectFit: "cover"}} />
                            </div>
